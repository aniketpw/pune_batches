            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 border-b border-[#E2E1DA] pb-2.5">
                  ...
                  {/* Status Indicator (Driven by Column O: Done vs Pending) */}
                  <div>
                    {item.isDone ? (
                      <span className="px-2 py-0.5 rounded-[2px] bg-emerald-50 text-emerald-800 border border-emerald-200 text-[8.5px] font-black uppercase tracking-wider flex items-center gap-1">
                        <CheckCircle2 className="w-2.5 h-2.5 text-emerald-600" />
                        <span>Announced (Done) ✓</span>
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-[2px] bg-amber-50 text-amber-800 border border-amber-300 text-[8.5px] font-black uppercase tracking-wider flex items-center gap-1 animate-pulse">
                        <AlertTriangle className="w-2.5 h-2.5 text-amber-600" />
                        <span>⚠️ Notice Pending</span>
                      </span>
                    )}
                  </div>
                </div>
                ...
                {/* Collapsible WhatsApp Announcement Notice (Hidden by default, shown on Share or Preview) */}
                {expandedNoticeId === item.id && (
                  <div className="bg-[#FAF9F5] border border-emerald-300/90 rounded-[2px] p-2.5 space-y-1.5 animate-in fade-in zoom-in-95 duration-150 shadow-2xs">
                    <div className="flex items-center justify-between">
                      <span className="text-[8.5px] font-black text-emerald-800 uppercase tracking-wider flex items-center gap-1">
                        <MessageSquare className="w-3 h-3 text-emerald-600" />
                        Column K Announcement (WhatsApp Message)
                      </span>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="text-[8.5px] font-bold text-slate-400 hover:text-slate-700 uppercase cursor-pointer"
                          title="Hide preview"
                        >
                          ✕ Hide
                        </button>
                    ...
                    <div className="text-[11px] text-slate-800 bg-white border border-[#E2E1DA] rounded-[2px] p-2.5 leading-relaxed whitespace-pre-line font-sans select-all max-h-36 overflow-y-auto">
                      {item.announcement}
                    </div>
                  </div>
                )}
                ...
                {/* Card Action Footer */}
                <div className="pt-2 border-t border-[#E2E1DA] flex flex-wrap items-center gap-2">
                  {/* Share on WhatsApp Button */}
                  <button
                    type="button"
                    onClick={() => handleShareWhatsApp(item)}
                    className="flex-1 min-w-[130px] py-1.5 sm:py-2 px-3 text-[9px] sm:text-[10px] font-black uppercase tracking-wider bg-emerald-600 hover:bg-emerald-700 text-white rounded-[2px] flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-xs active:scale-95"
                    title="Send notice directly to WhatsApp"
                  >
                    <MessageSquare className="w-3.5 h-3.5" />
                    <span>Share on WhatsApp</span>
                  </button>

                  {/* Toggle Preview / Copy Notice */}
                  <button
                    type="button"
                    onClick={() => {
                      if (expandedNoticeId === item.id) {
                        setExpandedNoticeId(null);
                      } else {
                        handleCopyAnnouncement(item);
                        setExpandedNoticeId(item.id);
                      }
                    }}
                    className="py-1.5 sm:py-2 px-2.5 text-[9px] sm:text-[10px] font-black uppercase tracking-wider bg-white hover:bg-[#FAF9F5] text-slate-700 border border-[#E2E1DA] rounded-[2px] flex items-center justify-center gap-1 transition-all cursor-pointer active:scale-95"
                    title="Preview announcement text or copy"
                  >
                    {expandedNoticeId === item.id ? (
                      <>
                        <EyeOff className="w-3 h-3 text-slate-500" />
                        <span>Hide Notice</span>
                      </>
                    ) : (
                      <>
                  ...
                  <button
                    ...
                    disabled={isUpdating}
                    className={`py-1.5 sm:py-2 px-3 text-[9px] sm:text-[10px] font-black uppercase tracking-wider rounded-[2px] flex items-center justify-center gap-1.5 transition-all cursor-pointer active:scale-95 disabled:opacity-50 ${
                      item.isDone
                    ...
                    title="Toggle announced status in Column O in Google Sheet"
                  >
                    {isUpdating ? (
                      <>
